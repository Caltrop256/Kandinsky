...
public class DoNothingButThrowErrorOnNonNumericValuesOperator extends UnaryPrefixOperatorNode {
    public DoNothingButThrowErrorOnNonNumericValuesOperator(CharInfo pos, Node right) {
        super(pos, right);
    }
    ...
    public Thunk eval(AbstractState frame, Scope scope, Continuation cont) throws RuntimeException {
        return this.right.eval(frame, scope, ContUtils.wrap(res -> {
            if(res.isInteger()) return frame.cont(cont, new IntegerValue(0 + res.getInteger()));
            if(res.isFloat()) return frame.cont(cont, new FloatingPointValue(0 + res.getFloat()));
            throw new TypeError(this, res.type, "You may only apply the DoNothingButThrowErrorOnNonNumericValues Operator on numeric types!");
        }));
    }
