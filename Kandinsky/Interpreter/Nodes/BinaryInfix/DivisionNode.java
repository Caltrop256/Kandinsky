...
public class DivisionNode extends BinaryInfixOperatorNode {
    static public final OperatorInfo info = new OperatorInfo("/", 20);

    public DivisionNode(CharInfo pos, Node left, Node right) {
    ...
    public Thunk eval(AbstractState frame, Scope scope, Continuation cont) throws RuntimeException {
        return ContUtils.overload(left, right, frame, scope, cont, new TypeSplitter(info.symbol, this) {
            ...
            public Value run(IntegerValue left, IntegerValue right) throws RuntimeException {
                if(right.getInteger() == 0) throw new RuntimeException(this.caller, "Division by 0!");
